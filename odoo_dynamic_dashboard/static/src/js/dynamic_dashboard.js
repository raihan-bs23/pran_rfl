...
export class OdooDynamicDashboard extends Component {
    ...
    setup() {
        ...
        this.CampaignSelector = useRef('CampaignSelector');
    ...
    /* Function for rendering the dashboard */
        ...
        await this.orm.call('verbex.ai.campaign', 'get_campaign_records', [[]]).then(function (response) {
            response.forEach((ev) => {
                const options = document.createElement("option");
                options.value = ev.id;
                options.text = ev.name;
                self.CampaignSelector.el.append(options)
            });
        })
        await this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId, parseInt(self.CampaignSelector.el.value)]).then( function (response){
    ...
    campaignFilter(){
        $(".items").empty();
        var self = this;
        let campaign_id = $("#campaign-selector").val();
        this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId, parseInt(campaign_id)]).then( function (response){
            for (let i = 0; i < response.length; i++) {
                if (response[i].type === 'tile'){
                    mount(DynamicDashboardTile, $('.items')[0], { props: {
                        widget: response[i], doAction: self.action, dialog:self.dialog, orm: self.orm
