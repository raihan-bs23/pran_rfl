...
class DashboardBlock(models.Model):
    ...
    def get_dashboard_vals(self, action_id, campaign_id=None, start_date=None, end_date=None):
        """Fetch block values from js and create chart"""
        block_id = []
        if campaign_id != 0:
            campaign_ids = self.env['verbex.ai.campaign'].browse([campaign_id]).ids
        else:
            campaign_ids = self.env['verbex.ai.campaign'].search([('state', '=', 'published')]).ids
        dashboard_block = self.env['dashboard.block'].sudo().search([('client_action_id', '=', int(action_id))])
        campaign_model = ['campaign.retailers', 'post.call.analysis', 'sale.order']
        for rec in dashboard_block:
            if rec.filter is False:
                rec.filter = "[]"
            filter_list = literal_eval(rec.filter)
            filter_list = [cond for cond in filter_list if cond[0] not in  ['campaign_id', 'id']]
            # Customized Domain for Campaign
            if rec.model_name == "verbex.ai.campaign":
                filter_list.append(('id', 'in', campaign_ids))
            if rec.model_name in campaign_model:
                filter_list.append(('campaign_id', 'in', campaign_ids))
            ...
            if rec.model_name:
                if rec.type == 'graph':
                    ...
                    vals.update({'x_axis': x_axis, 'y_axis': y_axis, 'value': float_round(total_value, 2)})
                else:
                    self._cr.execute(self.env[rec.model_name].get_query(domain,
                                                                        rec.operation,
                                                                        rec.measured_field_id,
